...
public:
    Mat stitchRetina(Mat &centerImage, Mat &topImage,
        ...
        for( size_t i = 1; i < 5; ++i ) {
            ...
            //cout << imagesName[i] << " -> " << imagesName[0] << endl;  //offending line
            
            m_transform[i] = match(m_greenChannels[0], m_greenChannels[i], reducedMask); //offending line
            printMatrixInfo(i);
        }
        ...
        for( int i = 1; i <= 4; ++i) {
            ...
            //cv::Rect newArea = getProjectedArea(m_sizes[i].height, m_sizes[i].width, m_transform[i]); //offending line
